...
class MarketSearchActivity : AppCompatActivity() {
    ...
    fun icon(distance: Int, a: MartData) {
        ...
        val ggg1 = PolylineOptions().color(Color.GREEN)
            .add(LatLng(a.latitude.toDouble(), a.lontitude.toDouble()))
    ...
    fun inita() {
        ...
        binding.button.setOnClickListener {
            ...
            for (a in MartList) {
                val distances =
                    getdistance(alpa, beta, a.latitude.toDouble(), a.lontitude.toDouble())
                if (distances <= 2000)
                //전방 2KM이내 검색해주기
                {
                    if (a.latitude == "") {
